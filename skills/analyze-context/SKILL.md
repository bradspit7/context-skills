...
**A machine switch alone does not need the full read.** After a switch, `device-sync` pulls, bootstraps and syncs memory, then invokes this skill with the argument `arrival`; Step 1 then routes to the short arrival briefing. An explicit "catch me up" / "brief me" / "full briefing" still gets the full briefing.
...
When the skill is invoked with the argument `arrival` (device-sync does this), run it as `bash ~/.claude/skills/analyze-context/scripts/currency-check.sh --arrival` instead. In that mode the two reasons every machine switch produces (a handoff a day or more old, a stamp naming the other machine or none) no longer force the full briefing, and commits that landed after the handoff are listed by subject instead of forcing it; FINDINGs, the multi-dev pattern and a never-committed handoff still force it.

One run replaces the old Step 0 + Step 1.5 + pre-briefing checkpoint. It reports: machine identity, pattern markers, fetch + new remote branches, newer sibling worktrees, unpulled upstream, a branch survey (10 most recent refs ∪ last-24h refs) comparing the handoff doc's blob hash, all-refs handoff commits from the last 7 days with HEAD-reachability, lag (days + commits since the doc was last touched), header stamps, per-dev handoff drift NOTEs (multi-dev, non-blocking), and the out-of-repo memory path (with a linked-worktree warning when memory must key off the main checkout).
...
- **`DEPLOY PARITY` section (printed only when the project has a `deploy-parity.json`)** → carry its one line into the briefing header verbatim. It is the only source for any claim about production (current, behind, which tree is live). It is a state, not a FINDING; `could not check` is reported, never dropped.
- **`UPGRADES`, `CI:` and `RULED OUT` sections (printed only when there is something to say)** → state lines, never FINDINGs. `UPGRADES`: pending `DOCKET-INBOX-*.md` filings at this project's root are handbacks to process this session; a full `UPGRADE-QUEUE.md` goes into Known issues; recent `Upgrade:` sentences go under Recently shipped. `CI:` goes into the header; a red or unreadable CI is an open item, never a footnote. `RULED OUT` lists the owner's standing rulings — carry them into Locked decisions and **never re-propose one**.
- **`SAME-DAY RESUME CANDIDATE` (RESUME CLASS section) + zero FINDINGs → take the slim path.** Unless the user explicitly asked for a full briefing ("catch me up", "brief me", "what's the state") or their message implies new scope: read the primary doc fully and deliver `analyze-handoff`'s 3-line summary (last completed / next intended / blocker) **plus the docket** (open items by ID, one line each, preserving each row's status marker so a parked/deprioritized row is never shown as actionable; skip only rows moved to the handoff's Resolved/✅ section; source it from the doc's own next-tasks/open-items section — or the separate docket file the handoff points to, if any. **Bound that file: `wc -c` first; at or under ~40KB read it fully, above that read only its open-item region — header plus open/next-tasks, stopping at the first Resolved/Archived/Closed heading — and state the remainder as a number (`381KB of closed history not read`), never a silent omission. Step 3's size valve applies on the slim path too, even though Steps 2–4 are skipped; full rule in `analyze-handoff` Step 2**) with a one-line "full briefing on request" offer — do NOT run Steps 2–4's deep reads. This in-skill downgrade IS the analyze-context↔analyze-handoff routing; invocation-time routing measurably never happens (2026-07-04 audit: 0 slim-path uses in 204 sessions while 20+ same-day resumes paid the full read). A `FULL BRIEFING` class, any FINDING, or an explicit full-picture request proceeds normally — the class line's stated reasons say which.
- **`ARRIVAL BRIEFING` (RESUME CLASS section, printed only under `--arrival`) + zero FINDINGs → take the arrival path.** Unless the user explicitly asked for a full briefing: read the primary doc as the class block's `MEASURED` line says (whole at or under ~40KB, otherwise only its current-state portion, with the unread remainder stated as a number) and bound the docket exactly as the slim path does. **Skip Steps 2 through 4.5:** no depth reads (memory topic files, specs, archive), no helper agents or subagents, no refutation cross-check, no test or verify runs. **One exception: Step 4.5's this-machine row still runs.** When the Blocked line or a docket row names a past action of this machine, or says something lives only on it, check this machine's runtime logs and that day's session transcripts as that row says, in the main thread, before listing it; ask the owner only what they do not answer. Deliver this short format, then stop:

  ```markdown
  ## <Project> — arrival
  **Machine:** <name> · **HANDOFF:** written on <machine> <N>h ago (<M> commits behind HEAD) · **Sync:** <device-sync's one line: pulls, bootstrap, restart needed or not>
  <the UPGRADES / CI / RULED OUT / DEPLOY PARITY lines, verbatim, whichever were printed>
  **Last done:** <one line> · **Next:** <one line> · **Blocked:** <one line, or "nothing">
  **Docket:** <open items by ID, one line each, status markers kept; the unread remainder as a number>
  Full briefing on request: say "brief me".
  ```

  A `FULL BRIEFING` class, any FINDING, or an explicit full-briefing request proceeds normally.
...
- **Per-dev handoff drift is a non-blocking `NOTE`, not a FINDING.** On a multi-dev repo the survey reports `NOTE perdev-divergent` when a surveyed ref carries per-dev `HANDOFF-<dev>.md` content outside HEAD's history. A teammate's divergence is normal — ignore it. But a NOTE naming *your own* resolved `HANDOFF-<dev>.md` (Step 2's identity) means a newer copy of your file lives on that ref: treat it as a currency signal for that file — read/re-derive from there — even though it never blocks synthesis.
- **No-git projects:** the script falls back to file mtimes. Doc >7 days old → flag staleness in the briefing header; in-content dates become the staleness evidence. **The `RESUME CLASS` signal is emitted here too** — derived from the doc's mtime instead of its last commit, and its clean line says so, including that no commits-behind check exists without git. It used to be emitted only from the git branch, so the slim-path downgrade was **structurally unreachable** on a no-git project and every such project paid the full deep read at every session start, forever. Both branches now call one shared emitter, so the routing this skill calls *mechanical* actually is.
...
- **≥ ~60KB → delegate.** Dispatch parallel read-only `lean-reader` subagents (Read/Grep/Glob only, no instruction files preloaded), each assigned a slice of the read set. Where that type is not installed, use general-purpose and tell it in the prompt to read only — it keeps write and shell tools and loads every instruction file. **Not Explore:** its own description says it reads excerpts rather than whole files, which is the skim Law 2 forbids. A lean-reader has no shell, so any file whose authoritative copy is not the working tree — it exists only on another branch, or Step 1 named a newer copy on another ref — is extracted by the main thread first (`MSYS_NO_PATHCONV=1 git show "<ref>:<path>" > <scratchpad file>`, then confirm its byte count is not zero) and handed over by that path. Put this contract in the prompt:
  - Read every assigned file **fully**, chunking past every truncation; never sparse-search.
  - Return a structured extract as **numbered sections, in this order**: current-state claims (with dates), locked decisions, open items/blockers, gotchas/rules relevant to upcoming work, contradictions against the HANDOFF summary you supply in the prompt, then the two below.
  ...
  - Return every **limitation or capability claim as its verbatim source sentence with its line number**, never a paraphrase — a summary that rewords a limitation routinely flips its scope ("only mode A is restricted" where the source restricts both), and the briefing then promises what the tool refuses.
  - **Deliver it as ONE final message under ~12,000 characters.** To fit, shorten the verification quotes and the prose of the other sections first; never shorten a limitation or capability sentence, or a quote offered as evidence that something is absent. If it still will not fit after shortening, send it over budget: never drop an entry, a section, or a file's verification quotes to meet the budget, because the head check below catches a split and nothing catches a drop. Only an agent's last message reaches the parent: an answer that runs past the output cap, which counts the agent's reasoning as well as its answer, is split, and its beginning is lost with no marker.
  Synthesize from the extracts, **checking each one first, head before tail.** A result that starts mid-sentence or lacks its leading sections was split at the output cap: resume that agent (SendMessage) for the missing sections rather than re-running the read. Only once the head is intact: it must contain each numbered section, and a missing bottom-third quote means the file wasn't fully read — re-dispatch. The budget lowers the odds of a split; this check is what catches one. This keeps a 200K-token corpus out of the main context (~15-25K after delegation) without re-enabling the skim failure.
...
**Targeted docket lookup — bind the subject to its standing dispositions.** The moment the session's work subject is known (the user's first message names an artifact, or the handoff's entry point implies one), search every docket file for that artifact's name/slug — `python ~/.claude/skills/analyze-context/scripts/docket_corpus.py search -i '<slug>' --scope here` (it covers the project's real docket paths, row bodies kept in a separate file, archives and pending inbox filings, and prints what it searched) — **before forming any recommendation about it** — and re-run the lookup whenever the subject changes mid-session. An oversized docket guarantees a read horizon, and nothing else catches a standing disposition buried below it: the session-start read covers the top sections, not the line where the project already decided the thing you're about to re-recommend. (Measured cost of skipping: two full analysis cycles and a near-miss destructive redirect change, re-deriving a recommendation the docket had already dispositioned against — the deciding line sat below every session-start read horizon in a 230KB docket.) The lookup is ~200ms; run it as a reflex, not a judgment call.
...
Step 1 proved you have the newest *files*. This step proves the briefing's *claims* are true. It runs after the reads and before the briefing; skip it on the slim path (a `SAME-DAY RESUME CANDIDATE` run — the 3-line summary makes no current-state assertions worth re-deriving) and on the arrival path (an `ARRIVAL BRIEFING` run — a short read by design; the full briefing is one request away), which runs only the this-machine row below.
...
| a "production is CURRENT / live / at HEAD" claim | the **parity sweep** — the Step 1 `DEPLOY PARITY` line runs it (`scripts/deploy-parity.py`, opted into by `deploy-parity.json`); without that config, do it by hand: fetch every served artifact and compare each to the blob at the named ref, then name the one tree production actually matches. **Never a single-page hash:** a page that did not change in the deploy matches either way, so the probe is drawn from outside the affected set and reports clean while production is many commits behind. |
| an open item or owner question about an action **this machine** took (or a thing said to live only on it — `LIVE on <machine>`). This is the this-machine row; it runs on the arrival path too | this machine's own record first: the project's gitignored runtime logs and that day's session transcripts under `~/.claude/projects/<slug>/` — then ask the owner only what they do not answer |
| a recorded LIMITATION or CAPABILITY claim, in either direction and with its scope words — *"X cannot be done on this machine"*, *"only mode A is restricted"*, an owner's "manual step" recorded after a tool refusal | **the tool itself: spend one call attempting X** — through a different tool form than the one recorded as failing, for an action the owner has authorized — or grep the guard in source, before carrying the claim forward or planning around it |

**A carried blocker is a dated claim about tooling, and it is the one claim nobody re-tests.** Every other row above governs asserting a capability you have not re-verified; a limitation entry is the inversion — asserting an *INcapability* — and it survives structurally rather than through sloppiness, because its own framing discourages the experiment that would refute it, and being wrong about it costs only *deferred* work rather than a visible failure. So when the context layer surfaces a standing blocker that is **gating real work**, re-test it before each deferral rather than inheriting it. **The tell is a constraint carried forward verbatim across sessions while the thing it blocks keeps accumulating.** (Measured: a project's HANDOFF, docket, and carried constraints all named the same standing blocker — *"the Browser pane does not composite, screenshots cannot be produced here"* — and a house rule requiring before/after screenshots was deferred across multiple sessions and five commits of brand-visible work. A different browser MCP, same machine, same session, composited on the first attempt: the blocker was true of one *tool*, generalized into a property of the *environment*, then hardened into a gate on the *work*.)
...
**Production:** <the DEPLOY PARITY line, verbatim — only when the project declares one> · **CI:** <the CI line, verbatim — only when printed>
```
...
- Write anything — read-only skill; writes are `update-context`'s job (a temporary extract into the session scratchpad for a reader, per Step 4, is not a context-layer write)
...
- **`analyze-handoff`** — slim sibling: same-day resumption, reads only the handoff doc, 3-line summary. Escalates to `device-sync` on machine switch (arrival pull + memory sync first, then this skill's arrival briefing); here on >24h gap or any cross-branch signal.
- **`device-sync`** — the arrival command. Ends by invoking this skill with `arrival`, which gives the short arrival briefing; the full briefing only on request.
