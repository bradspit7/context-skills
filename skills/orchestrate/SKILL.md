...
- **Exactly-one source coverage** — every finding maps to exactly one input; a missing / duplicated / invented mapping forces the fallback (no silent drop, no double-count). This includes the receive-mode parse of a pasted review: a NUMBERED review's parse must be proven against its own numbering ({1..N} exactly once — a bare count check passes on drop+duplicate, observed live), with unproven coverage surfaced as a logged hole, never silence.
...
- **Read-only lenses** — every non-orchestrator agent in the fleet (reviewers, verifiers, critics, synthesizers, parse/transform helpers) runs under a read-only `agentType` (e.g. `Explore`) AND is told in-prompt to avoid mutating shell commands (`Explore` keeps `Bash`); a gate that can mutate the artifact under review is not a gate. The orchestrator applies confirmed fixes itself, after adjudication. A read-only `agentType` also pins its own default **model** — an omitted `model` resolves to the agent-type's default (measured: `Explore` → Haiku), never the session model — so every lens/verifier call passes its model explicitly (the fleet's role defaults or a per-lens pin).
