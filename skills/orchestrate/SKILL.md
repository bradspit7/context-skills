...
- **Read-only lenses** — every non-orchestrator agent in the fleet (reviewers, verifiers, critics, synthesizers, parse/transform helpers) runs under a read-only `agentType` (e.g. `Explore`) AND is told in-prompt to avoid mutating shell commands (`Explore` keeps `Bash`); a gate that can mutate the artifact under review is not a gate. The orchestrator applies confirmed fixes itself, after adjudication.
