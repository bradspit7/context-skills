...
- **NOT `reflect-upgrades`.** That skill is reactive + tooling-focused (did this session teach us something that should become a hook/skill/rule?) and routes durable learnings to the upgrades pipeline; this scan is proactive + work-focused and routes existing open work to orchestrate. The two can co-exist in a session — if you surface a tooling/upgrade idea here, leave it for reflect-upgrades.
