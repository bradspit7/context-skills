...
- **`reflect-upgrades/`** — after substantial work, decides where each lesson should act (reuse an existing tool, fix the code, a check the project already runs, a shared guard, a path rule, a skill step, or a note) and, at every `update-context` wrap, builds exactly one — anything bigger goes to the project's `UPGRADE-QUEUE.md`. Also nudged mid-session by a once-per-session `UserPromptSubmit` hook bundled under the skill (`hooks/upgrade-reflection-nudge.py`); nudge and manual firings route and queue but never build. Bundles `scripts/capability-index.py` (find an existing tool by what it does).
