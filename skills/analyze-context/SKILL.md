...
| an open item or owner question about an action **this machine** took (or a thing said to live only on it — `LIVE on <machine>`) | this machine's own record first: the project's gitignored runtime logs and that day's session transcripts under `~/.claude/projects/<slug>/` — then ask the owner only what they do not answer |
