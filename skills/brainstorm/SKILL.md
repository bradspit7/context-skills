...
- **Heavyweight fan-out** — only when the design is heavyweight (multi-subsystem, novel architecture, or the user says "go deep"; otherwise explore inline as above): run the private exploration as an orchestrate read-fan-out — 3-5 independent generate-only design alternatives (blind, no self-ranking) → name the kill-criteria for THIS design first (pattern fit, verification cost, scope/YAGNI) → independent refuters, killing only on an unrebutted criterion violation (uncertainty = a RISK flag, not a kill) → one completeness pass ("what did we miss?") before synthesis. The whole fan-out completes before any user contact — the output is still exactly one recommended design. No subagent capacity → disciplined inline pass; degrade, never abort, never a reason to skip the HARD-GATE. _(First 2-3 firings run unratified: note token cost and whether the refuters actually killed or reshaped anything; tighten the gate if it fired on a design that didn't need it.)_
